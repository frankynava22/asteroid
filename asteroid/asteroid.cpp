/*
This game was created with the help of onelonecoder! he created the game engine header.
*/
...
protected:
	...
	{
		//how the shape looks, aka the size of the ship
		vecModelShip =
		{
			{ 0.0f, -5.0f},
			{-2.5f, +2.5f},
		...
		//create semi circular circle
		//collision is checked with perfect circ le
		int verts = 20;
	...
	void ResetGame()
	{
		//this is where the play starts on the screen
		//we start it off centered
		...
		//initial asteroids, we start with two.
		//each line is an asteroid and its properties.
	...
	//this is for different screens. it helps fit the gui better
	...
	//our game engine olcConsoleGameEngine.h calls this function
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		if (bDead)
			ResetGame();

		//this clears the screen for a new game if player died
		Fill(0, 0, ScreenWidth(), ScreenHeight(), PIXEL_SOLID, 0);

		//this is how the play steers the ship using the arrow keys
		if (m_keys[VK_LEFT].bHeld)
			player.angle -= 5.0f * fElapsedTime;
		if (m_keys[VK_RIGHT].bHeld)
			player.angle += 5.0f * fElapsedTime;

		//this is the acceleration, the user uses the up arrow to move forward
		...
		//if the ship crashes with the asteroid, then end the game
		for (auto& a : vecAsteroids)
			if (IsPointInsideCircle(a.x, a.y, a.nSize, player.x, player.y))
				bDead = true; 

		//this allows player to shoot bullets in the direction the tip of the ship is facing
		if (m_keys[VK_SPACE].bReleased)
			vecBullets.push_back({ 0, player.x, player.y, 50.0f * sinf(player.angle), -50.0f * cosf(player.angle), 100.0f });

		//here we draw the asteroids and update them if they are destroyed 
		for (auto& a : vecAsteroids)
		{
			// VELOCITY changes POSITION (with respect to time)
			...
			//we make sure the asteroids are located within the screen the user has, we do not want it going out
			WrapCoordinates(a.x, a.y, a.x, a.y);

			//draw asteroids
			DrawWireFrameModel(vecModelAsteroid, a.x, a.y, a.angle, (float)a.nSize, FG_RED);
		}
		...
		// add more asteroids if no more asteroids. the player wins in this scenario
		if (vecAsteroids.empty()) 
		{
			...
			// Add two new asteroids, but in a place where the player is not. it is three more asteroids in this case
	...
	void DrawWireFrameModel(const vector<pair<float, float>>& vecModelCoordinates, float x, float y, float r = 0.0f, float s = 1.0f, short col = FG_BLUE)
	{
